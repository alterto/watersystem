...
/*========================================================================
	...
	Params:		[lpszConnect]: 连接字符串, 包含连接信息.
		...
		adAsyncConnect			异步方式打开连接. Ado用 ConnectComplete 事
						件来通知已经完成连接. 
	odbc数据源连接字符串样式：
	Provider=MSDASQL.1;Password=sa;Persist Security Info=True;User ID=sa;Data Source=tian;Initial Catalog=ksxt
==========================================================================*/
BOOL CAdoConnection::Open(LPCTSTR lpszConnect, long lOptions)
{
	...
	if (strcmp((const char *)lpszConnect, "") != 0)
	{
...
/*========================================================================
	...
	Remarks:	请参考 CAdoRecordSet 类的Open方法. 返回的 Recordset 对象始
			终为只读、仅向前的游标.
==========================================================================*/
_RecordsetPtr CAdoConnection::Execute(LPCTSTR lpszSQL,VARIANT *RecordsAffected, long lOptions)
{
	ASSERT(m_pConnection != NULL);
	...
	try
	{
		return m_pConnection->Execute(_bstr_t(lpszSQL), RecordsAffected, lOptions);
	}
	catch (_com_error e)
	{
...
CString CAdoConnection::GetDefaultDatabase()
{
	...
	try
	{
		return CString(LPCTSTR(m_pConnection->GetDefaultDatabase()));
	}
	catch (_com_error e)
	{
...
CString CAdoConnection::GetProviderName()
{
	...
	try
	{
		return CString(LPCTSTR(m_pConnection->GetProvider()));
	}
	catch (_com_error e)
	{
...
CString CAdoConnection::GetVersion()
{
	...
	try
	{
		return CString(LPCTSTR(m_pConnection->GetVersion()));
	}
	catch (_com_error e)
	{
