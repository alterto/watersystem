...
BOOL CAdoRecordSet::Open(LPCTSTR strSQL, long lOption, CursorTypeEnum CursorType, LockTypeEnum LockType)
{
	...
	{
		if (IsOpen()) Close();
		BOOL bRet = SUCCEEDED(m_pRecordset->Open(_variant_t(LPCTSTR(m_strSQL)), 
											_variant_t((IDispatch*)m_pConnection->GetConnection(), true),
											CursorType, LockType, lOption));
		//先尝试连接一下
		if (!bRet)
		{
			bRet = m_pConnection->Open();
			if (bRet)
			{
				SetAdoConnection(m_pConnection);
				bRet = SUCCEEDED(m_pRecordset->Open(_variant_t(LPCTSTR(m_strSQL)),
					_variant_t((IDispatch*)m_pConnection->GetConnection(), true),
					CursorType, LockType, lOption));
			}
		}
		...
		{
			//先尝试重新连接一次
			if (IDYES == ::MessageBox(NULL, _T("服务器连接断开，是否尝试诚信连接？"), _T("提示"), MB_YESNO))
			{
				while (!m_pConnection->Open())
				{
					if (IDYES != ::MessageBox(NULL, _T("服务器连接断开，是否尝试诚信连接？"), _T("提示"), MB_YESNO))
						return bRet;
				}
...
// 		TRACE(_T("%s\r\n"), GetLastError());
		BOOL bRet = FALSE;
		CString strError;
		strError.Format(_T("Warning: 打开记录集发生异常. 错误信息: %s;\r\n Last Error:%s"), e.ErrorMessage(),GetLastError());
		...
		if (!bRet)
		{
			if (IDYES == ::MessageBox(NULL, _T("服务器连接admin断开，是否尝试诚信连接？"), _T("提示"), MB_YESNO))
			{
				while (!m_pConnection->Open())
				{
					if (IDYES != ::MessageBox(NULL, _T("服务器连接断开，是否尝试诚信连接？"), _T("提示"), MB_YESNO))
...
BOOL CAdoRecordSet::IsFieldAtUp(LPCTSTR lpFieldName)
{
	try
	{
		_variant_t vt = m_pRecordset->Fields->GetItem(lpFieldName)->Properties->Item["ISAUTOINCREMENT"]->Value;
		return (vt.boolVal == VARIANT_TRUE);
	}
	catch (_com_error e)
	{
		TRACE(_T("Warning: IsFieldAtUp 方法发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
		return FALSE;
	}
}

BOOL CAdoRecordSet::IsFieldAtUp(long index)
{
	try
	{
		_variant_t vt = m_pRecordset->Fields->GetItem(_variant_t(index))->Properties->Item["ISAUTOINCREMENT"]->Value;
		return (vt.boolVal == VARIANT_TRUE);
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::Find(LPCTSTR lpszFind, SearchDirectionEnum SearchDirection)
{
	...
	try
	{
		if (strcmp((const char *)lpszFind, "") != 0)
		{
			m_strFind = lpszFind;
		}
